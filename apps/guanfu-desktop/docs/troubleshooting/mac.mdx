...
2. If the app doesn't work, try deleting all its files and folders:
		...
		3. Here's a list of files and folders you can safely delete:
			...
			- ``~/Library/Saved Application State/org.upscaleProc.Guanfu.savedState/``
			- ``~/Library/Group Containers/W2T4W74X87.org.upscaleProc.Guanfu``
			- ``~/Library/Containers/Guanfu`` and other folders there that have `guanfu` in their name.
			- ``~/Library/Preferences/org.upscaleProc.Guanfu.plist/``
			- ``~/Library/Preferences/org.upscaleProc.Guanfu.helper.plist/``
