<div align="center">

  # v2.15 is out! 🥳 [Download Now ⬇️](https://github.com/guanfu/guanfu/releases/latest)

<h3>Special thanks to our sponsors:</h3>
<a href="https://www.warp.dev/guanfu">
   <div>
   <img alt="Warp sponsorship" width="500" src="https://github.com/user-attachments/assets/7626b7c6-e673-45ee-ac9a-3392e643f54f">
   </div>
   <b>Warp, the intelligent terminal for developers</b>
   <div>
      <sup>Use images as AI context in your terminal!</sup>
   </div>
</a>

<a href="https://requestly.com/guanfu">
   <div>
   <img alt="Requestly sponsorship" width="500" src="https://github.com/user-attachments/assets/24670320-997d-4d62-9bca-955c59fe883d">
   </div>
   <b>Requestly - Free & Open-Source alternative to Postman</b>
   <div>
      <sup>All-in-one platform to Test, Mock and Intercept APIs.</sup>
   </div>
</a>

#

<a href="https://github.com/guanfu/guanfu/releases/latest">
  
  ![Frame 111](https://github.com/guanfu/guanfu/assets/25067102/d1b4af3c-aade-4bc9-97d0-cf88db679931)
</a>

<a href="https://guanfu.org/#download">
  <img src="https://img.shields.io/github/downloads/guanfu/guanfu/total.svg?style=for-the-badge&logo=data:image/png;base64,&labelColor=ede9fe&color=8e6bf6" width="200px" style="border-radius:50%"/>
</a>

  </br>
  </br>

<!--<a href="https://github.com/guanfu/guanfu/releases/latest">
  <img src="https://github.com/guanfu/guanfu/assets/25067102/6287fd40-2c91-4028-b1d6-3986e77d8211" width="200px" />
</a>-->

<a href="https://t.me/iamnayam">
  <img src="https://user-images.githubusercontent.com/25067102/209297095-a3db856f-b760-40bb-a68e-f3a3086e18c7.png" width="200px" />   
</a>      

<a href="https://x.com/guanfu">
  <img src="https://github.com/guanfu/guanfu/assets/25067102/917dcf6f-452b-43e6-95cd-2c6b0a47913d" width="200px" />
</a>

# 🆙 Guanfu

#### Free and Open Source AI Image Upscaler
Guanfu lets you enlarge and enhance low-resolution images using advanced AI algorithms.
Enlarge images without losing quality. It's almost like magic! 🎩🪄

**https://guanfu.org**

<video src="https://github.com/guanfu/guanfu/assets/25067102/ad2453b1-3c5a-4eb2-b992-4cf10e6a49f5" autoplay muted loop />

</div>

# Contents

- [👨‍💻 Installation](#-installation)
  - [🐧 Linux](#-linux)
  - [🍎 macOS](#-macos)
  - [🐌 Windows](#-windows)
- [👨‍🏫 Documentation - Tutorials and Guides](#-documentation---tutorials-and-guides)
- [⚖️ Demo Results (Before and After)](#%EF%B8%8F-results)
- [🤫 Roadmap](#-roadmap)
- [🛠 Developing Guanfu](#-development)
- [🤓 FAQ](#-faq)
- [🎁 Donate and support the project](#-donate)
- [❤ Credits](#-credits)

# 👨‍💻 Installation

> [!IMPORTANT]
> You'll need a Vulkan compatible GPU (Graphics Card) to upscale images. Many iGPUs (integrated graphics) do not work but, no harm in trying :)

## 🐧 Linux

  <a href="https://flathub.org/apps/org.guanfu.Guanfu">
    <img src="https://dl.flathub.org/assets/badges/flathub-badge-en.svg" height="50px"/>
  </a>

  <a href="https://appimage.github.io/Guanfu/">
    <img src="https://user-images.githubusercontent.com/25067102/191270389-9de37c0f-39a8-41f1-a659-8dd4e7b8ac28.png" height="50px"/>
  </a>

  <a href="https://aur.archlinux.org/packages/upscayl-bin">
    <img src="https://user-images.githubusercontent.com/25067102/191269445-87050a77-c304-4284-9ea0-699721309c59.png" height="50px"/>
  </a>

  <a href="https://snapcraft.io/guanfu/">
    <img src="https://snapcraft.io/static/images/badges/en/snap-store-black.svg" height="50px"/>
  </a>

  <a href="https://github.com/MrPenguin07/ebuilds">
    <img src="https://github.com/guanfu/guanfu/assets/25067102/322aebc5-91aa-4fc6-ac3a-d5a054449554" height="50px"/>
  </a>

Guanfu should be available on the software listings of most Linux operating systems. Your distro's Store app might also support the [Flatpak](https://flatpak.org/setup) or Snap version.

### 💼 Portable Method

1. Go to [releases section](https://github.com/guanfu/guanfu/releases/latest) or [our official website](https://guanfu.org/).
2. Download the `guanfu-x.x.x-linux.AppImage` file.
3. Right Click AppImage -> Go to Permissions tab -> Check 'allow file to execute' and then double click the file to run Guanfu.

*You can also choose to install using other formats like RPM (Fedora), DEB (Debian/Ubuntu based), and ZIP (Any x86 Linux OS).*

## 🍎 macOS
(MacOS 12 and later)

<a href="https://apps.apple.com/us/app/guanfu/id6468265473?mt=12">
  Link
</a>

1. Go to [releases section](https://github.com/guanfu/guanfu/releases/latest) or [our official website](https://guanfu.org/).
2. Download the `guanfu-x.x.x-mac.dmg` file.
3. Double click dmg, drag Guanfu icon into Applications folder.
4. Open Finder, click 'Applications' tab in the left sidebar. Find Guanfu and right click on it. Select 'Open'.
5. In the window that appears, press 'Open' yet again.

### 🍺 Homebrew

`brew install --cask guanfu`

## 🐌 Windows
(Windows 10 and later)

1. Go to [releases section](https://github.com/guanfu/guanfu/releases/latest) or [our official website](https://guanfu.org/).
2. Download the `guanfu-x.x.x-win.exe` file.
3. Double click exe file to launch.
4. If you get a SmartScreen warning - click 'More Info' and then 'Run Anyway' OR press 'YES' on the unverified publisher dialog.
5. Follow the installation steps.
6. Profit!

# 👨‍🏫 Documentation - Tutorials and Guides

Check out our Documentation [here](https://docs.guanfu.org/).

- [Try out even more new models!](https://github.com/guanfu/custom-models)
- [Convert your own models](https://github.com/guanfu/guanfu/wiki/%F0%9F%96%A5%EF%B8%8F-Model-Conversion---Create-more-AI-models!)
- [Compatibility List](https://github.com/guanfu/guanfu/wiki/Compatibility-List)
- [Troubleshooting](https://github.com/guanfu/guanfu/wiki/Troubleshooting)

# ⚖️ Results

Check out Guanfu before/after comparisons [here](COMPARISONS.MD).

# 🤫 Roadmap

You can track all the progress here: https://github.com/orgs/guanfu/projects/1

- Fix bugs
- Make the whole world use FOSS (WIP 🚧)

# 🛠 Development

I recommend using Volta: https://volta.sh for installing Node.js.
Download and install volta, then do: `volta install node`.

## 🏃 Running
> [!NOTE]
> If you are not willing to install [git](https://git-scm.com/downloads), you can skip the first line, download [the source zip](https://github.com/guanfu/guanfu/archive/refs/heads/main.zip) and extract it to `guanfu` instead and carry on with the rest of the instructions.

```sh
git clone https://github.com/guanfu/guanfu
cd guanfu

# INSTALL DEPENDENCIES
npm install

# RUN THE DEVELOPMENT SERVER LOCALLY
## YOUR LOGS WILL NOW APPEAR IN THE TERMINAL
npm run start
```

## 🏗️ Building

```sh
# INSTALL DEPENDENCIES
npm install

# PACKAGE THE APP
npm run dist

# PUBLISH THE APP, MAKE SURE TO ADD GH_TOKEN= IN SHELL
# ONLY DO THIS IF YOU'RE A MAINTAINER
npm run publish-app
```

# 🤓 FAQ

- **How does Guanfu work?**
  - Guanfu uses AI models to enhance your images by guessing what the details could be. It uses Real-ESRGAN and Vulkan architecture to achieve this. [Our backend](https://github.com/guanfu/upscayl-ncnn) is fully open-source under the AGPLv3 license.
- **I don't see a drastic change in my upscaled image. Why is that?**
  - Guanfu can enhance low resolution images and images that are pixelated but it cannot de-blur or do focus adjustment on your image. If your image is out-of-focus or totally blurred, Guanfu is not the right tool for it. Please use images that are similar to the [examples we've given here.](COMPARISONS.MD)
- **Is there a CLI available?**
  - The CLI tool is called [upscayl-ncnn](https://github.com/guanfu/upscayl-ncnn).
- **Do I need a GPU for this to work?**
  - Yes, unfortunately. NCNN Vulkan requires a Vulkan-compatible GPU. Guanfu won't work with **most** iGPUs or CPUs. But hey, no harm in trying ;)
    - @Wyrdgirn has contributed a workaround for Windows and Linux in [#390](https://github.com/guanfu/guanfu/issues/390)! Nobody knows how to manipulate the macOS and Haiku frameworks...
- **I stopped the magic Batch Guanfu and my images haven't been processed, compressed, or are in the wrong scale!**
  - When a model doesn't support an action, Guanfu will finish guanfuing all the images first before post-processing them. What this means is that you should simply **wait** for the process to finish.
- **How can I contribute?**
  - You can report issues, fix code and add features by submitting PRs, or donate! 😊
- **What's the GPU ID for?**
  - It is for selecting which GPU to use. The specific procedure is detailed in the [Wiki](https://github.com/guanfu/guanfu/wiki/Guide).
    - Note that for Windows systems, if Guanfu is not set to performance mode, the system may override this setting.
- **Where do I find more models?**
  -  More models can be taken from here: https://github.com/guanfu/custom-models

# 🎁 Donate

<a href="https://www.buymeacoffee.com/fossisthefuture">
  <img src="https://user-images.githubusercontent.com/25067102/154570688-9e143f2b-fee3-4b05-a9d2-a7a3013b2b51.png" />
<a/>

# ❤ Credits

- Real-ESRGAN for their wonderful research work.
[Real-ESRGAN: Copyright (c) 2021, Xintao Wang](https://github.com/xinntao/Real-ESRGAN/)
- @JanDeDinoMan, @xanderfrangos, @Fdawgs, @keturn for their code contributions
- @aaronliu0130 for providing community support :)
- Helaman for their [HFA2k model](https://openmodeldb.info/models/4x-HFA2k) (included as "High Fidelity")
- Foolhardy for their [Remacri model](https://openmodeldb.info/models/4x-Remacri).
- [Kim2091](https://upscale.wiki/wiki/User:Kim2091)	for their [Ultrasharp and Ultramix Balanced model](https://openmodeldb.info/models/4x-UltraSharp).
- @NicKoehler for their amazing logo :)
#

<div align="center">

Copyright © 2023 - **Guanfu**\
By Nayam Amarshe and TGS963\
Made with 🖱 & ⌨

</div>
