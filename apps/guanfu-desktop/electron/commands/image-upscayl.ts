...
import { getBackendModel, getBackendScale } from "../../common/backends";
...
const imageUpscayl = async (event, payload: ImageUpscaylPayload) => {
  ...
  const scale = getBackendScale(payload.backendId, payload.scale);
  const useCustomWidth = payload.useCustomWidth;
  const customWidth = useCustomWidth ? payload.customWidth : "";
  const model = getBackendModel(payload.backendId, payload.model as string);
