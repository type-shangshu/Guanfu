...
cmake -S "$BACKEND/src" -B "$BUILD"   -DNCNN_VULKAN=ON   -DNCNN_VULKAN_ONLINE_SPIRV=ON   -DNCNN_ENABLE_LTO=OFF   -DCMAKE_BUILD_TYPE=Release
cmake --build "$BUILD" --target upscayl-bin -j"${JOBS:-4}"
